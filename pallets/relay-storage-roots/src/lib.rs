...
pub mod pallet {
	...
	/// This must use the `Identity` hash to allow efficiently removing the oldest item.
	#[pallet::storage]
	pub type RelayStorageRoot<T: Config> =
		CountedStorageMap<_, Identity, RelayBlockNumber, H256, OptionQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_relay_storage_root() {
			...
			if <RelayStorageRoot<T>>::count() > T::MaxStorageRoots::get() {
				let first_key = <RelayStorageRoot<T>>::iter_keys().next().unwrap();
