...
// If the runtime does not implement cumulus_primitives_core::RelayParentOffsetApi, we default to 1.
const DEFAULT_RELAY_PARENT_OFFSET: u32 = 1;
...
use crate::{
	...
	NimbusApi, LOG_TARGET,
};
use async_backing_primitives::{Slot, UnincludedSegmentApi};
...
> {
	...
	pub para_slot_duration: Option<Duration>,
		...
		+ 'static,
{
	async move {
		...
		let overseer_handle = relay_client
			.overseer_handle()
			.expect("Overseer handle should be available. If it fails, collation submission won't work either.");
		let mut connection_helper =
			crate::collators::BackingGroupConnectionHelper::new(keystore.clone(), overseer_handle);
		...
		loop {
			...
			let relay_parent_offset = para_client
				.runtime_api()
				.relay_parent_offset(best_hash)
				.unwrap_or(DEFAULT_RELAY_PARENT_OFFSET);

			let para_slot_duration = slot_timer.slot_duration();
			...
			let Some(para_slot) = adjust_para_to_relay_parent_slot(
				...
				relay_chain_slot_duration,
				SlotDuration::from_millis(para_slot_duration.as_millis().saturated_into::<u64>()),
			) else {
				continue;
			};
			...
			connection_helper.update(para_slot.slot).await;
			...
			let (parachain_inherent_data, other_inherent_data) = match collator
				.create_inherent_data_with_rp_offset(
					...
					(), // CIDP context (unit type for slot-based collator)
				)
				.await
			{
				Err(err) => {
...
fn adjust_para_to_relay_parent_slot(
	...
	let new_slot = Slot::from_timestamp(
		relay_slot.timestamp(SlotDuration::from_millis(
			relay_chain_slot_duration.as_millis() as u64,
		))?,
		para_slot_duration,
	);
	let para_slot = SlotInfo {
		slot: new_slot,
		timestamp: new_slot.timestamp(para_slot_duration)?,
	};
	tracing::debug!(
		target: LOG_TARGET,
...
/// * `Ok(None)` - If an epoch change is detected in the relay parent or its ancestors
...
/// If any block in this chain contains a BABE epoch change digest, it returns `None` to skip
/// building on this relay parent (as collations built on old session blocks will be rejected).
	...
	RelayClient: RelayChainInterface + Clone + 'static,
{
	...
	// Check for epoch change in the best block
	if sc_consensus_babe::contains_epoch_change::<RelayBlock>(&relay_header) {
		tracing::debug!(
			target: LOG_TARGET,
			relay_block_hash = %relay_best_block,
			relay_block_num = %relay_header.number(),
			"Skipping block building - epoch change detected in best relay block."
		);
		return Ok(None);
	}
	...
	while required_ancestors.len() < relay_parent_offset as usize {
		let Ok(Some(next_header)) = relay_client
			.header(BlockId::Hash(*relay_header.parent_hash()))
			.await
		else {
			return Err(());
		};

		// Check for epoch change in ancestor blocks
		if sc_consensus_babe::contains_epoch_change::<RelayBlock>(&next_header) {
			tracing::debug!(
				target: LOG_TARGET,
				relay_block_hash = %next_header.hash(),
				relay_block_num = %next_header.number(),
				"Skipping block building - epoch change detected in relay ancestor."
			);
			return Ok(None);
		}
	...
	let Ok(Some(relay_parent)) = relay_client
		.header(BlockId::Hash(*relay_header.parent_hash()))
		.await
	else {
		return Err(());
	...
	// Check for epoch change in the relay parent itself
	if sc_consensus_babe::contains_epoch_change::<RelayBlock>(&relay_parent) {
		tracing::debug!(
			target: LOG_TARGET,
			relay_block_hash = %relay_parent.hash(),
			relay_block_num = %relay_parent.number(),
			"Skipping block building - epoch change detected in relay parent."
		);
		return Ok(None);
	}
