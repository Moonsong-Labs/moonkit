...
where
	...
	Client: ProvideRuntimeApi<Block>
		...
		+ Sync
		+ 'static,
	Client::Api: NimbusApi<Block> + RelayParentOffsetApi<Block> + UnincludedSegmentApi<Block>,
...
pub(crate) async fn determine_core<H: HeaderT, RI: RelayChainInterface + 'static>(
	...
	let (selector, core_index) = if is_new_relay_parent {
		let Some(core_index) = cores_at_offset.get(0) else {
			return Ok(None);
		};
		...
		let Some(core_index) = cores_at_offset.get(selector) else {
			return Ok(None);
		};
