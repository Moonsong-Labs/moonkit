	...
	P::Signature: TryFrom<Vec<u8>> + Member + Codec,
{
	...
	pub fn adjust_authoring_duration(&mut self, authoring_duration: Duration) -> Option<Duration> {
		let slot_duration = SlotDuration::from_millis(self.slot_duration().as_millis() as u64);
	...
	pub async fn wait_until_next_slot(&mut self) -> Result<(), ()> {
		...
		let (time_until_next_attempt, mut next_para_slot) =
			self.time_until_next_block(SlotDuration::from_millis(slot_duration.as_millis() as u64));
