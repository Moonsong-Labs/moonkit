...
#[derive(Debug)]
pub(crate) struct SlotTimer<Block, P> {
	...
	para_slot_duration: Option<Duration>,
...
/// - `Slot`: The Nimbus slot used for authoring
fn compute_next_wake_up_time(
	para_slot_duration: Duration,
	relay_slot_duration: Duration,
	...
	let mut block_production_interval = min(para_slot_duration, relay_slot_duration);

	if assigned_core_num > para_slots_per_relay_block && para_slot_duration >= relay_slot_duration {
		block_production_interval = max(
			relay_slot_duration / assigned_core_num,
	...
	let para_slot = Slot::from_timestamp(
		timestamp,
		SlotDuration::from_millis(para_slot_duration.as_millis().saturated_into()),
	);
	(duration, para_slot)
}
...
fn time_until_next_attempt(
	...
	(
		...
		Timestamp::from(next_slot_time as u64),
	)
}

impl<Block, P> SlotTimer<Block, P>
where
	P: Pair,
	...
	P::Signature: TryFrom<Vec<u8>> + Member + Codec,
{
	...
	pub fn slot_duration(&self) -> Duration {
		self.para_slot_duration
			.unwrap_or(self.relay_slot_duration.into())
	}
	...
	pub async fn wait_until_next_slot(&mut self) -> Result<(), ()> {
		...
		let (time_until_next_attempt, mut next_para_slot) = compute_next_wake_up_time(
			slot_duration,
		...
		match self.last_reported_slot {
			...
			Some(ls) if ls + 1 < next_para_slot && next_para_slot <= ls + 3 => {
				next_para_slot = ls + 1u64;
			}
			None | Some(_) => {
		...
		tracing::debug!(
			target: LOG_TARGET,
			?slot_duration,
			para_slot = ?next_para_slot,
			"New block production opportunity."
		);

		self.last_reported_slot = Some(next_para_slot);
		Ok(())
	}
}
...
mod tests {
	...
	fn test_get_next_slot(
		...
		let (wait_duration, _) = compute_next_wake_up_time(
			para_slot_duration.as_duration(),
			relay_slot_duration,
			core_count,
			time_now,
			offset,
