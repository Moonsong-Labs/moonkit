...
/// A utility struct for writing collation logic that makes use of Nimbus entirely
/// or in part. See module docs for more details.
///
/// The `CIDPContext` type parameter specifies the context type passed to
/// `CreateInherentDataProviders`. Common choices are `()` or
/// `(PHash, PersistedValidationData, NimbusId)`.
pub struct Collator<Block, P, BI, CIDP, RClient, Proposer, CS, CIDPContext = ()> {
	...
	proposer: Proposer,
	collator_service: CS,
	_marker: std::marker::PhantomData<(Block, Box<dyn Fn(P) + Send + Sync + 'static>, CIDPContext)>,
}

impl<Block, P, BI, CIDP, RClient, Proposer, CS, CIDPContext>
	Collator<Block, P, BI, CIDP, RClient, Proposer, CS, CIDPContext>
where
	Block: BlockT,
	RClient: RelayChainInterface,
	CIDP: CreateInherentDataProviders<Block, CIDPContext> + 'static,
	CIDPContext: Clone + Send + 'static,
...
{
	/// Instantiate a new instance of the `Collator`.
	...
	pub async fn create_inherent_data_with_rp_offset(
		...
		let mut other_inherent_data = self
			...
			.create_inherent_data_providers(parent_hash, cidp_context)
